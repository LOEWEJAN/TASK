CLASS zjl_cl_attach_da DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tp_db     TYPE zjl_atao.
    TYPES: tp_db_key TYPE zjl_atao_key_s .    "DB key
    ...
    TYPES:
      tpt_db     TYPE SORTED TABLE OF tp_db     WITH UNIQUE KEY task_id vers_no atac_no.
    TYPES:
      tpt_db_key TYPE SORTED TABLE OF tp_db_key WITH UNIQUE KEY task_id vers_no atac_no .

    CONSTANTS:
      co_db TYPE c LENGTH 20 VALUE 'ZJL_ATAO' ##NO_TEXT.

    CLASS-METHODS get_instance
      RETURNING
        VALUE(rr_instance) TYPE REF TO zjl_cl_attach_da .
    ...
    METHODS read
      IMPORTING
        ...
        !i_include_buffer TYPE abap_boolean
      EXPORTING
        !et_db            TYPE tpt_db .
    ...
    METHODS get_next_atac_no
      IMPORTING
                !i_task_id       TYPE zjl_id
      RETURNING VALUE(r_atac_no) TYPE zjl_version .
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA or_instance TYPE REF TO zjl_cl_attach_da .
...
CLASS zjl_cl_attach_da IMPLEMENTATION.
  ...
  METHOD cud_prepare.
    ...
    FIELD-SYMBOLS: <ls_msg> TYPE zjl_message.
...
* Update der TMP-Buffer
  ...
  METHOD cud_copy.
    ...
    LOOP AT ot_buffer_c_tmp ASSIGNING FIELD-SYMBOL(<ls_buffer_c_tmp>).
      READ TABLE ot_buffer_c ASSIGNING FIELD-SYMBOL(<ls_buffer_c>) WITH TABLE KEY task_id = <ls_buffer_c_tmp>-task_id
                                                                                  ...
                                                                                  atac_no = <ls_buffer_c_tmp>-atac_no.
    ...
    LOOP AT ot_buffer_d_tmp ASSIGNING FIELD-SYMBOL(<ls_buffer_d_tmp>).
      DELETE ot_buffer_c WHERE task_id EQ <ls_buffer_d_tmp>-task_id
                           ...
                           AND atac_no EQ <ls_buffer_d_tmp>-atac_no.
      ...
      LOOP AT it_db ASSIGNING FIELD-SYMBOL(<ls_db>).
        ...
        LOOP AT ot_buffer_d ASSIGNING FIELD-SYMBOL(<ls_buffer_d>) WHERE task_id EQ <ls_db>-task_id
          ...
          DELETE et_db WHERE task_id EQ <ls_buffer_d>-task_id
                         ...
                         AND atac_no EQ <ls_buffer_d>-atac_no.
        ENDLOOP.
  ...
  METHOD _create.
    ...
    SELECT FROM (co_db) FIELDS task_id, vers_no, atac_no FOR ALL ENTRIES IN @it_db
           WHERE task_id EQ @it_db-task_id
             AND vers_no EQ @it_db-vers_no
             AND atac_no EQ @it_db-atac_no INTO CORRESPONDING FIELDS OF TABLE @lt_db_key.
  ...
  METHOD _delete.
    ...
    LOOP AT it_db ASSIGNING FIELD-SYMBOL(<ls_db_delete>) WHERE task_id EQ 0.
    ...
    LOOP AT it_db ASSIGNING <ls_db_delete> WHERE atac_no = 0.
      APPEND NEW zjl_cx_behavior( textid = VALUE #( msgid = zjl_if_rap=>co_msgid msgno = '001' attr1 = 'ATAC_NO' ) ) TO et_message.
      RETURN.
    ENDLOOP.
    ...
    IF i_no_delete_check EQ abap_false.
      SELECT task_id, vers_no, atac_no FROM (co_db) FOR ALL ENTRIES IN @lt_db
                                       WHERE task_id EQ @lt_db-task_id
                                         AND vers_no EQ @lt_db-vers_no
                                         AND atac_no EQ @lt_db-atac_no INTO CORRESPONDING FIELDS OF TABLE @lt_db_key.
    ENDIF.
    ...
    LOOP AT lt_db ASSIGNING <ls_db_delete>.
      ...
      INSERT VALUE #( task_id = <ls_db_delete>-task_id
                      vers_no = <ls_db_delete>-vers_no
                      atac_no = <ls_db_delete>-atac_no ) INTO TABLE ot_buffer_d_tmp.
    ENDLOOP.
  ...
  METHOD save.
    ...
    DELETE zjl_atao FROM TABLE @( CORRESPONDING #( ot_buffer_d ) ).

  ENDMETHOD.

  METHOD get_next_atac_no.

    SELECT MAX( atac_no ) INTO @r_atac_no FROM (co_db) WHERE task_id EQ @i_task_id.

    r_atac_no = r_atac_no + 1.
  ...
  METHOD _determine.
    ...
    CASE i_change_mode.
      WHEN zjl_if_rap=>co_modstat_create.
        IF cs_db-atac_no IS INITIAL.
          cs_db-atac_no          = get_next_atac_no( i_task_id = cs_db-task_id ).
        ENDIF.
